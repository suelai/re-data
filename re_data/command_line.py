...
from re_data.utils import parse_dbt_vars, load_metadata_from_project, normalize_re_data_json_export, get_project_root

from re_data.notifications.utils import (
    build_notification_identifiers_per_model,
    prepare_exported_alerts_per_model,
    validate_alert_types,
    ALERT_TYPES,
    create_owners_to_models_map,
    create_models_to_alerts_map,
)
...
def get_target_paths(kwargs, re_data_target_dir=None):
    ...
    dbt_target_path = os.path.join(project_root, project_dict["target-path"])
...
def generate(start_date, end_date, interval, re_data_target_dir, force, **kwargs):
    ...
    if dbt_vars:
        command_list.extend(["--vars", yaml.dump(dbt_vars)])
    add_dbt_flags(command_list, kwargs)
    completed_process = subprocess.run(command_list)
    completed_process.check_returncode()

    # export tests history
    tests_history_args = {"start_date": start_date, "end_date": end_date, "tests_history_path": tests_history_path}
    tests_history_command_list = [
        "dbt",
        "run-operation",
    ...
    if dbt_vars:
        tests_history_command_list.extend(["--vars", yaml.dump(dbt_vars)])
    ...
    table_samples_args = {"start_date": start_date, "end_date": end_date, "table_samples_path": table_samples_path}
    table_samples_command_list = [
        "dbt",
        "run-operation",
    ...
    if dbt_vars:
        table_samples_command_list.extend(["--vars", yaml.dump(dbt_vars)])
    ...
    if dbt_vars:
        dbt_docs.extend(["--vars", yaml.dump(dbt_vars)])
    ...
    print(f"Generating overview page", chalk.green("SUCCESS"))


@click.option("--port", type=click.INT, default=8085, help="Specify the port number for the UI server. Default is 8085")
@click.option(
    "--re-data-target-dir",
    type=click.STRING,
    help="""
...
def slack(start_date, end_date, webhook_url, subtitle, re_data_target_dir, select, send_all_good, **kwargs):
    ...
    # Load dbt manifest to get tags
    dbt_manifest_path = os.path.join(re_data_target_path, "dbt_manifest.json")
    tags_by_model = {}
    ...
    alerts_per_model = prepare_exported_alerts_per_model(
        alerts=alerts, members_per_model=slack_members, selected_alert_types=selected_alert_types
    )
    ...
    for model, details in alerts_per_model.items():
        ...
        tags = tags_by_model.get(model, [])
        slack_message = generate_slack_message(model, details, owners, subtitle, selected_alert_types, tags)
    ...
    for email in email_to_models_map:
        ...
        if email != "NO_OWNER":
            ...
            mime_msg = build_mime_message(
                ...
                html_content=render.render_email_alert(
                    alerts=model_to_alerts_map, owner=email, models_to_notify=all_models
                ),
            )
